...
// icons
import { IconMinus, IconPlus, IconSearch, IconX } from '@tabler/icons'
...
const AddNodes = ({ nodesData, node }) => {
  ...
  /**
   * Filter nodes based on 'OpenAI' and 'Clova'
   */
  const filterNodesData = () => {
    const filteredNodes = nodesData.filter(
      nd => nd.name.toLowerCase().includes('openai') || nd.name.toLowerCase().includes('clova')
    )
